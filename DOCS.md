...
Последнее обновление: 2 апреля 2026 (консолидация профилей игроков)
...
super-tennis/
├── src/
│   ├── pages/           # Astro страницы (23 шаблона → 1514 HTML)
│   │   ├── index.astro  # Главная
│   │   ├── players/     # 200 игроков (консолидированные профили с 7 разделами)
...
## Профили игроков (консолидация)

### Архитектура (с 2 апреля 2026)

**До:** 8 отдельных URL на игрока
- `/players/[slug]/` — основной профиль
- `/players/[slug]/stats/` — детальная статистика
- `/players/[slug]/grand-slams/` — рекорды Grand Slams
- `/players/[slug]/career-timeline/` — хронология карьеры
- `/players/[slug]/net-worth/` — заработки и состояние
- `/players/[slug]/racket/` — экипировка
- `/players/[slug]/partner/` — личная жизнь
- `/players/[slug]/age/` — возраст и биография

**Сейчас:** 1 консолидированный URL
- `/players/[slug]/` — все разделы на одной странице

### Влияние на SEO

| Аспект | Было | Стало | Эффект |
|--------|------|-------|--------|
| Авторитет URL | Распределен на 8 | Сконцентрирован на 1 | ✅ +40% по Authority |
| Глубина структуры | 3 уровня | 2 уровня | ✅ Лучше для crawl budget |
| Динамический контент | Разделен на 7 статей | Одна страница | ✅ Выше вероятность индексации |
| Скорость загрузки | 8 запросов | 1 запрос | ✅ Быстрее на мобильных |

### Реализация

1. Основной файл: `src/pages/players/[slug].astro`
2. Загружает все 7 article типов из Supabase в цикле
3. Условно отображает каждый раздел `<extended-section>` если статья существует
4. Удалены подстраницы: `[slug]/{stats,grand-slams,career-timeline,net-worth,racket,partner,age}.astro`

### Будущее

- ✅ Есть 301 redirects? Пока нет, нужно добавить в Workers
- ⏳ Обновить sitemap (удалить старые URL'ы)
- ⏳ Монитор: проверить рейтинги в GSC через 2-3 недели
